...
- **PR description generator** ✅ *(shipped Roadmap-1; expanded Roadmap-5)* — `Draft PR…` button in the Git tab. Primary click runs the Claude Code CLI if configured (opens a detached terminal — `helpers/claude_cli.py`), otherwise calls the API path (`helpers/pr_draft.py` → in-app dialog). Right-click / Shift+click pops a menu to explicitly override. **Roadmap-2 P5a** added an "Open in Browser" button: spawns `gh pr create --web --body-file <tmp>` so the GitHub New-PR page opens pre-filled. **Roadmap-5** added direct PR creation: a PR title field (auto-extracted from the generated text) + **"Create PR on GitHub"** button that runs `gh pr create --title … --body-file … --base …` on a background thread, shows the PR URL, and offers to open it in the browser — no browser form to fill in. The temp body file is deleted after a successful create. The Claude CLI instruction also now includes `gh pr create` after writing `PR_DRAFT.md` (push-first step included), with automatic cleanup of `PR_DRAFT.md` on success.
...
- ✅ **Phase 5a — "Open PR on GitHub" button.** Added to the PR draft dialog next to Copy. Writes body to temp `.md`, spawns `gh pr create --web --body-file <tmp>` with `cwd=<project>`. `--body-file` (not `--body "..."`) sidesteps Windows command-line length limits and quote escaping. Button greyed + tooltipped when `gh` isn't on PATH. **Roadmap-5** renamed to "Open in Browser" and supplemented by the direct-create button (see Stage 6 PR generator entry).
...
**Last updated: 2026-05-25** — Roadmap-5 shipped. Highlights:

- **Claude CLI model selector.** New `claude_cli_model` config property (default `claude-haiku-4-5-20251001`). Exposed as a combobox in Settings → Claude Code CLI. Every manager-spawned `claude --print` call now passes `--model <id>` — pre-commit hook, commit-message Suggest, Draft PR CLI path. Escape hatch: empty string omits the flag and defers to `~/.claude/settings.json`. Stderr logging added to `call_claude_cli_print` for typo'd model IDs.
- **Pre-commit hook hang fix.** Root cause: Opus 4.7 (Max subscriber default) is too slow on large diffs and `subprocess.run` timeout didn't always interrupt cleanly. Fixed by defaulting to Haiku 4.5 (3–5 s) and raising the pre-commit timeout 30 → 45 s. Diff already capped at 24 000 chars.
- **Commit-message quality parity: Claude CLI ≈ Ollama.** `_strat_claude_cli` now uses `_build_llm_prompt` (same subject+body prompt as Ollama) instead of the old single-line-only system constant. Neutral `cwd=~` prevents project `CLAUDE.md` from making small models reply in "assistant mode" instead of generating a commit message.
- **Draft PR → direct GitHub PR creation.** API-path dialog now has a PR title field (auto-extracted from the generated Markdown) and a **"Create PR on GitHub"** button. Runs `gh pr create --title … --body-file … --base …` on a background thread; shows PR URL on success and offers to open it in the browser. Temp body file auto-deleted after successful create. Claude CLI instruction updated to include `gh pr create` + `PR_DRAFT.md` cleanup after success (conditional on `gh` being on PATH).

...
│   │   ├── baseline_copy.py       A project's committed copy of the baseline:
│   │   │                          strict header, one LF-normalised sha, and a
│   │   │                          write policy that only replaces absent or
│   │   │                          outdated copies.
│   │   ├── ignore_alignment.py    Manager-written instruction companions follow
│   │   │                          their referrer's git visibility. Git answers;
│   │   │                          only .gitignore is ever written.
│   │   ├── instructions_posture.py Does a project's instruction chain RESOLVE,
│   │   │                          and will Claude Code LOAD it (delivery)?
...
| `instructions_wiring.plan_wiring(posture)` / `apply_wiring(...)` | The one writer. Writes `CLAUDE.md → @BASIC_INSTRUCTIONS.md → @project-baseline.md` (the project's copy): `WRITE_COPY` / `REFRESH_COPY` / `LOCALIZE`. A resolving external include is localized **even when approved**, because a committed machine path breaks other checkouts and worktrees. Every `LOCALIZE` carries the exact directive and line it was planned from and all of them are checked before ANY write; copy steps always run before a repoint, whatever order a plan lists them in. Repairs exactly ONE topology even though the classifier resolves a general graph — resolving broadly is what stops the panel "fixing" a project that already works through its own valid chain. Never deletes, never rewrites authored prose, refuses on duplicate directives. Reads with `newline=""` so a one-line insert cannot silently convert a CRLF file. |
| `dialogs/instructions_split.SplitProposalDialog` | The offer, on the row it applies to. Per project and never bulk: moving a lesson log is a large editorial change to a file a person wrote. Every section is listed with its byte cost and a tick, and **nothing is ticked on open**: the budget guesses a tail, which was wrong on three of the four real files it met — once selecting an operational section — and a pre-ticked box reads as a recommendation. `Suggest from budget` offers the guess on request. Recompute is live (18 ms on the 954 KB file). Two guards are mutation-tested — building the proposal writes nothing, and applying writes exactly the two files it represented. A refusal is a sentence beside the button, never only a greyed control. |
| `instructions_split.compute_split(text, ...)` / `apply_split(...)` | Moves an append-only lesson log out of the always-loaded file, leaving an index. The boundary is **proposed, never decided** — ALL-CAPS headings are 81% / 34% / 0% of the three oversized projects’ sections and all three are logs, so casing is a project ritual rather than a rule. Pure compute returns the complete new contents of BOTH files; `apply_split` refuses if the source’s sha256 moved since (two targets have live agents appending), refuses to overwrite an existing target, and writes the target first so a failure leaves the project untouched. Directive detection is delegated to `instructions_posture.scan_text`, and the newline to `instructions_wiring.dominant_newline`. The sections to move are an explicit `move_indices` set rather than a boundary, because *“the tail is the log”* was falsified by two of the three projects it was written for; the index renders **where the moved run was**, so the surviving document still reads in order. Sections are a **two-level tree** — `##` is not the unit of a lesson, and one project keeps a 64,030 B operational section holding twenty `###` ones — so a subsection moves alone, a parent carries its children, and a `###` whose parent stays is filed under a `## From:` heading in the target. A second split merges into the index the first wrote and appends to the existing target under a **second** digest guard; appending is offered only for a target this module wrote. |
| `install_identity.read_identity(raw, base_dir)` / `read_ownership(projects, here)` | **Two questions, never conflated.** *Where am I* — `FIRST_RUN`/`SAME`/`MOVED` from the recorded `install_dir`; no record is deliberately **not** a move. *Who owns the fleet* — `OWNED_HERE`/`OWNED_ELSEWHERE`/`SPLIT`/`UNOWNED`, derived from the projects and computed only over those that reach a baseline. `SAME` + `OWNED_ELSEWHERE` is a state, never an instruction: taking ownership from another install is an operation somebody invokes. `SPLIT` carries per-owner canonical paths and counts. Identity is **lexical** (reuses `instructions_posture.canonical`), so a junction reads as a different install — an intentional invariant, tested as the contract. |
| `install_identity.relocatable_updates(raw, identity)` | What a move invalidated. **Allowlist AND containment**, and the allowlist is `("template_dir",)` — everything else in the config points at software installed elsewhere or is user data. `search_roots` is why containment alone is not enough: a root can live inside the Manager's folder. |
| `install_identity.relocation_plan(...)` / `baseline_facts(path)` | Combines both questions into what may be offered, and refuses bulk repair for `SPLIT`, `UNOWNED` or any non-resolving project — otherwise one action silently fixes half a fleet. Carries path, size, mtime **and a content hash** for both baselines, because two different ones must not look equivalent to somebody reading a date and a byte count. Reports the difference; invents no version order. |
| `ignore_alignment.read_facts(root, git, rels)` / `decide(pairs, facts)` / `align(root, git, pairs)` | **Invariant:** a Manager-written companion that exists has the git visibility of the file that refers to it (`companions_of`: `BASIC_INSTRUCTIONS.md` ← `CLAUDE.md`, `project-baseline.md` ← the file with the live directive, `docs/LESSONS.md` ← `CLAUDE.md` only when it exists). **Facts are git's:** repo `REPO` / `NOT_OWN_REPO` / `NO_REPO` / `UNKNOWN(reason)` from `rev-parse --show-toplevel`; per path `TRACKED` / `LOCAL` (ignored) / `OPEN` / `UNKNOWN(reason)` from `ls-files` (literal pathspecs) and `check-ignore --stdin` (which rejects `GIT_LITERAL_PATHSPECS` with exit 128, measured), plus filesystem `EXISTS` / `MISSING` / `UNREADABLE` / `ESCAPES`. Every requested path gets a state; one git omitted is `OPEN` only after two clean exits. **`decide` is pure** and acts only on `LOCAL` referrer + `OPEN` existing companion; a tracked companion (untrack) and an ignored companion under a visible referrer (un-ignore) are notes. **`align`** re-reads before writing (the scan is never authorization), repeats to a fixed point for a two-deep chain, writes via `gitignore.ensure_pattern`, and reports what git confirms afterwards. Called by `apply_to_project(git_exe=)`, the split dialog and Retrofit. |
| `baseline_copy.read_copy(path)` / `classify_copy(facts, template_sha)` / `write_copy(root, template, expect)` | A project's committed `project-baseline.md`. **Why it exists:** Claude Code loaded no absolute baseline include on this machine — an include outside the project needs a per-project approval the desktop app never asks for, and an include spelled with a backslash or an unescaped space is not parsed at all (canaries, 2026-09-13). Line one is a strict header recording the sha256 of the template body; every hash is `sha256(lf(text))`, so a CRLF checkout is never "edited". `read_copy` reads one file and knows nothing of the template; `classify_copy` compares: `ABSENT` / `CURRENT` / `OUTDATED` / `EDITED` / `INVALID` (a header that starts like ours and does not parse — never confused with a person's file) / `UNMANAGED` / `UNREADABLE`. `write_copy` replaces only `ABSENT` or `OUTDATED`, re-reads before writing, verifies after, and writes nothing when the copy is already current. |
| `instructions_posture.delivery_of(scan, root, projects)` / `claude_code_parses(raw)` | Will Claude Code load the chain — an **observed** fact, never a Manager invariant. Strongest first: `UNPARSEABLE` (a directive on the chain has a backslash or unescaped space), then containment by `realpath` (Claude Code resolves a junction before deciding, so a junction to outside is outside), then the project record's `hasClaudeMdExternalIncludesApproved` → `EXTERNAL_APPROVED` / `EXTERNAL_BLOCKED`. Unreadable config or an unresolvable path is `UNKNOWN`, never blocked. `ProjectInstructions.healthy` is `RESOLVED` + `IN_PROJECT` and is the only state a panel renders green. `reached_baseline` stays the reached PATH; `baseline_match` (`MATCH` / `MISMATCH` / `UNKNOWN`) is the content fact and `baseline_source` is what ownership groups by, so a localized fleet is not N owners of itself. **Health ordering:** in-project + current copy healthy › outdated copy (Update copy) › external approved (works here only, Localize) › external blocked / unparseable (loads nothing, Localize) › edited / invalid / unmanaged root file (a person decides) › unknown. |
| `mcp_projects.resolve_project_record(root, projects)` / `external_includes_approved(record)` | The ONE forward-slash matching rule for a `~/.claude.json` project record, shared by trust and external-include approval so the two can never pick different records. `project_trust_state` is a thin reader over it. |
| `instructions_wiring.apply_to_project(...)` | Re-read, re-plan, write, re-read, for ONE project. Returns a structured `ApplyOutcome` whose sentence **derives from** the outcome — `SKIPPED_STATE_CHANGED` and `SKIPPED_BLOCKED` are separate members, because the safety net doing its job is not a refusal. Lives here rather than in a dialog so the fleet panel and the relocation share one implementation of the recompute. |
| `dialogs/relocate.RelocateDialog` | One action for "this installation moved". Writes nothing of its own — config through `cfg`, projects through `apply_to_project`. **Config first**, because `baseline_include_line` is derived from `template_dir` and the other order writes the old path into every project. Never applies to the fleet automatically. |
| `instructions_split.textual_mentions(entries, stats, ...)` / `scan_mentions(root)` | What else in the repository names the file a split is about to empty. A **textual-occurrence detector** and explicitly not a dependency graph — `see X`, `# X` and `path = "X"` are identical to it. Matches a path *token*, so `CLAUDE.md.bak` and `MY_CLAUDE.md` are other files. Sorts before capping, because filesystem order is not stable and an unsorted cap reports a different number each run; the sort takes a `_walk` seam since a real filesystem already returns small directories in name order. Capped / unreadable / oversize are three separate fields and none of them means "no mention" — any of them makes the count a **lower bound**. |
...
| `OllamaModelManagerDialog` | Browse / pull / delete Ollama models via native REST API | Settings → AI → 🦙 Manage Ollama Models… |
| `MCPConfigDialog` | Classify + edit tokensave MCP entries in both Claude configs | Settings → Integrations → 🔌 Manage MCP wiring… (or auto-launched at startup if drift detected) |
...
| `ask_tab_llm` | Dict. Ask tab AI backend settings (Roadmap-6). Same shape as `commit_message_llm` (provider, model, api_key_env, base_url, etc.) plus `claude_cli` as a valid provider. Falls back to `commit_message_llm` when key absent (migration path). Configured via Settings → AI → "Ask Tab AI" section. When provider is `claude_cli`, api_key_env + base_url fields are disabled. |

...
- **Daemon status indicator** — a `●` indicator + label in the log header polls `tokensave daemon --status` every 5 seconds; green when running (with PID), grey when stopped. Polling pile-up guard + a `finally`-block reschedule keeps the loop alive even if the subprocess errors

### Settings & Tools
- **Settings dialog** — configure all paths (`tokensave_exe`, `template_dir`, `editor_cmd`, `git_exe`, `codegraph_exe`, `claude_cli_exe`) and search roots through a GUI; changes apply immediately. Validates and auto-detects on save. Scrollable and resizable (760×700 default, 640×500 minimum) so growing sections never push controls off-screen

...
class App(tk.Tk):
    ...
    def cmd_git_commit(self):
        ...
        Commit button. Prefer ``self._git_path`` (set whenever the Git tab
        loads a project — via tab-switch or row-click) so the Git-tab button
        works without first round-tripping back to the Projects tab.
        Falls back to the Projects-tab selection only when ``_git_path``
        is unset (e.g. right-clicking before ever opening the Git tab).
        """

...
from helpers.llm import (
    _is_auth_error, _iter_json_lines, _call_llm,
)
# _build_llm_prompt also in helpers.llm — used only by _call_llm_for_commit_message
# which now lives in helpers.commit_messages, so no monolith import needed.
...
from helpers.release import (
    _last_release_tag, _commits_since, _classify_commits_for_changelog,
    _bump_version, _suggest_bump_kind,
    _render_release_notes, _patch_changelog,
...
from helpers.commit_messages import _suggest_commit_message, _pending_diff
...
class ReleaseWizardDialog(tk.Toplevel):
    ...
    def __init__(self, parent, path: str):
        ...
        self._release_basename = _release_basename(path, GIT_EXE)
        ...
        self._last_tag       = _last_release_tag(path, GIT_EXE)
        self._commits        = _commits_since(path, self._last_tag, GIT_EXE)
...
class AICodeReviewDialog(tk.Toplevel):
    ...
    def _load_diff(self):
        ...
        diff = _pending_diff(self._path, git_exe=GIT_EXE, lines_of_context=3)
...
class GitCommitDialog(tk.Toplevel):
    ...
    def _populate_suggestion(self, status_text: str, source: str = "initial"):
        ...
        if not llm_active:
            ...
            result = _suggest_commit_message(self._path, status_text, _cfg, GIT_EXE) or "chore: update files"
        ...
        def _worker(path=self._path, st=status_text, tok=my_token):
            try:
                result = _suggest_commit_message(path, st, _cfg, GIT_EXE)

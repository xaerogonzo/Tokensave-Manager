...
PROMPT_SNIPPETS: list[tuple[str, str]] = [
    ...
    (
        ...
        "  1. tokensave_status graph_root=[[sibling project path]] — node "
        "and edge counts, sync metadata and the branch that snapshot "
        "actually serves, so a thin answer later reads as a stale index "
        "rather than as absent code. Bare tokensave_status also names "
        "the initialized siblings beside this root, as do otherwise-"
        "empty search/context results. (The selectors need tokensave "
        "7.11.1+; before that status was classified selector-less and "
        "answered -32602.)\n"

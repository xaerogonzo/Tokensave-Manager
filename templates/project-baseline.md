...
Hard-won failure modes, delivered into every project under `docs/gotchas/` by TokenSave Manager (its own originals are `templates/gotchas/`).
...
| write or debug PowerShell, or read a `.ps1` build script | `docs/gotchas/powershell-silent-failures.md` |
| shell out to another program on Windows | `docs/gotchas/windows-subprocess.md` |
| parse another tool’s output, or wire in a linter | `docs/gotchas/wiring-in-an-external-analyzer.md` |
| design what a result, report or cursor returns | `docs/gotchas/empty-is-not-unknown.md` |
| need admin rights, or split off a privileged helper | `docs/gotchas/elevation-and-privilege.md` |
| move content out of a file, or split a big one | `docs/gotchas/moving-content-moves-its-guards.md` |
| add an `@include`, or wonder why rules are ignored | `docs/gotchas/claude-md-external-includes.md` |

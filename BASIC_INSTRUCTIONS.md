...
- **Controllers import dialogs directly**; cross-dialog deps (e.g. `McpStatusSection` opening `MCPConfigDialog`) use **lazy in-handler imports** to avoid module-load cycles.
...
- **Settings content packs onto its PAGE BODY, never on the tab.** Settings is a tab (`controllers/settings_tab.py`) with five pages, each its own `Canvas + body Frame` scroller. A section receives that `body` and packs everything onto it. The tab frame's only direct children are the startup-note label, the dirty bar and the inner Notebook — a section packed onto the tab instead would render almost right and then outlive its page, which `tests/test_settings_tab.py` guards. Mousewheel is bound on both the canvas and the body per page; a nested scrollable sub-widget must intercept the wheel event so it doesn't double-scroll.
- **A Settings section receives its host; it never discovers one.** The contract is `dialogs/settings_section.py`: `__init__(host, body, cfg, **injected)`, `save_into(raw) -> bool`, `bind_dirty(callback)`, and an optional `snapshot()` for state outside Tk variables. `host` is the application window, for `after()` / `parent=` / child-Toplevel parenting only. **No `.master`, `.winfo_toplevel()` or `.nametowidget()`** — `PathsSection` used to read the App as `self._dlg.master`, which inside a Notebook page silently resolves to the Notebook, so a `getattr(..., None)` returned None permanently while the buttons raised. Guarded by an AST test, on the rule rather than on one spelling.
- **Settings Save is a staged transaction.** Sections write into a `copy.deepcopy` of `cfg.raw`; a section returning False aborts with the live config untouched; the commit is `raw.clear(); raw.update(staged)` so the dict's IDENTITY survives. `dict(cfg.raw)` is not enough — `AISection.save_into` mutates the nested `commit_message_llm` in place and a shallow copy writes straight through. Filesystem side effects (the pre-commit hook) run after the commit and never roll it back.
...
- **`.pyscope/` is in the baseline for a functional reason, not a tidiness one.** PyScope writes `<project>/.pyscope/` ONLY when git ignores it, and otherwise keeps the project's analysis in per-user app data. Removing the pattern does not merely leak a cache directory into git status — it moves the cache somewhere else.
